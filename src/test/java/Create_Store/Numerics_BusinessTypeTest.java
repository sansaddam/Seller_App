...
public class Numerics_BusinessTypeTest extends BaseCapabilitiesTest {
	@Test
	public void numerics() throws InterruptedException {
		...
		wd.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//android.widget.TextView[@text='Set Your PIN']")));
		 ...
		 WebElement businessType = driver.findElement(AppiumBy.xpath("//android.widget.EditText[@text='Enter business name']"));
		 businessType.sendKeys("152922");
		 driver.findElement(AppiumBy.xpath("//android.view.ViewGroup[@content-desc=' Save']")).click();
		 String actual = businessType.getText();
			int length=actual.length();
			if (actual.equalsIgnoreCase("Enter business name") || length == 0) {
